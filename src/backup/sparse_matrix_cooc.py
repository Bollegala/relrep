...
class COOC:
    ...
    def create_matrix(self, corpus_fname, pairs_fname, patterns_fname, prefix):
        """
        ...
        # Create matrix.
        M = scipy.sparse.lil_matrix((len(wpairs), len(patterns)), dtype=numpy.float)
        with open(corpus_fname) as corpus_file:
            ...
            for line in corpus_file:
                ...
                for i in range(0, len(L)):
                    for j in range(i + 1, len(L)):
                        wpair = (L[i], L[j])
                        if (wpair in wpairs) and ((j - i - 1) <= self.WINDOW_SIZE):
                            for pattern in self.extract_patterns(L[i:j]):
                                if pattern in patterns:
                                    M[wpairs[wpair], patterns[pattern]] += 1

        # We will remove word pairs and patterns that have zero total frequencies in the matrix.
        ...
        with open("%s.patids" % prefix, 'w') as patid_file:
            for (pattern, patid) in patterns.items():
                patid_file.write("%d\t%s\n" % (patid, pattern))
        # Write word-pair ids.
        with open("%s.wpids" % prefix, 'w') as wpid_file:
            for ((first, second), wpid) in wpairs.items():
...
def process():
    ...
    """
    corpus_fname = "../work/ukwac.corp.small"
    ...
    C.create_matrix(corpus_fname, cooc_pairs_fname, patterns_fname, prefix)
    pass
